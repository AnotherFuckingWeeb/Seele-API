...
export async function getProjectsByTitle(req: Request, res: Response) : Promise<Response> {
    const project: IProject = req.body;
    const conn = await connect();
    const projects = await conn.query('call getProjectsByTitle(?)', [project.title]);
...
export async function getProjectsByUserId(req: Request, res: Response) : Promise<Response> {
    const user_id = req.params.userID;
    const conn = await connect();
    const projects = await conn.query('CALL getProjectsByUserId(?)', [user_id]);
